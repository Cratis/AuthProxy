...
public class when_single_oidc_provider_is_configured : Specification
{
    ...
    void Establish()
    {
        ...
        authConfigMonitor.CurrentValue.Returns(new C.Authentication
        {
            OidcProviders =
            [
                new C.OidcProvider { Name = "Microsoft Entra" }
            ]
        });
    ...
    [Fact] void should_redirect_directly_to_provider_login_endpoint() =>
        _redirectContext.Response.Headers.Location.ToString()
            .ShouldEqual("/.cratis/login/microsoft-entra?returnUrl=%2Fhome");
}
