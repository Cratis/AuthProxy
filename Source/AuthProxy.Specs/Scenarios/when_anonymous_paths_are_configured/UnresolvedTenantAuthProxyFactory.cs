...
/// session — which is the only configuration in which <c language="text">TenancyMiddleware</c>'s refusal branch is reached
/// at all.
/// <para>
/// This exists because the default factory resolves a fixed tenant for every request, so
/// <c language="text">TenancyMiddleware</c> never gets to refuse and the scenario would stay green with that enforcement
