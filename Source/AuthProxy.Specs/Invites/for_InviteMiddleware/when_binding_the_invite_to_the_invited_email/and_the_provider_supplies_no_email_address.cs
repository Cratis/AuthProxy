...
/// <summary>
/// A GitHub account whose email is private supplies no address at all: <c language="text">/user</c> returns a null email, so
/// no <c language="text">email</c> claim is written and only <c language="text">preferred_username</c> — the login name — is left. That is not
