...
/// <summary>
/// Specifies that a version-one cookie cannot authorize a different legacy subject.
/// </summary>
public class with_a_changed_subject : a_legacy_v1_authorization_cookie
{
    bool _result;

    void Because() => _result = _cache.IsAuthorized(
        Replay(DateTimeOffset.UtcNow.AddMinutes(5).ToUnixTimeSeconds()),
        LegacyPrincipal("different-user"),
        TenantId);
