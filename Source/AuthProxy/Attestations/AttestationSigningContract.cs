...
/// </summary>
/// <param name="Issuer">The issuer written to the assertion's <c language="text">iss</c> claim, naming the AuthProxy deployment that signed it.</param>
/// <param name="Audience">The audience written to the assertion's <c language="text">aud</c> claim, naming the single application entitled to consume it.</param>
/// <param name="KeyId">The identifier of the resolved active signing key, written to the JWS <c language="text">kid</c> header so a verifier can select the matching public key.</param>
/// <param name="PrivateKeyPem">The PEM-encoded RSA private key belonging to <paramref name="KeyId"/>.</param>
/// <param name="Lifetime">The lifetime applied to the assertion, from which its <c language="text">exp</c> claim is derived.</param>
/// <remarks>
    ...
    TimeSpan Lifetime)
{
    ...
    /// <c language="text">LogDebug("{Contract}", contract)</c> would write the signing key to the log. This override exists so
