...
/// <param name="Scheme">The scheme every later decision treats as the browser's — which is what settles cookie <c language="text">Secure</c> and the proxy's own public origin.</param>
...
/// <param name="RemainingForwardedFor">What was left of <c language="text">X-Forwarded-For</c> after the middleware consumed from it.</param>
/// <remarks>
/// These four values are the whole of what forwarded headers can change, and every downstream consequence in
/// the proxy — the address recorded against a sign-in, the <c language="text">Secure</c> flag on eleven cookies, the OIDC
/// <c language="text">post_logout_redirect_uri</c>, the post-logout origin allow-list — is derived from them. Asserting on
